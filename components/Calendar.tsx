...
const Calendar: React.FC<CalendarProps> = ({ 
  ...
  return (
    <div className="bg-white p-2.5 sm:p-6 pb-8 rounded-2xl sm:rounded-3xl border border-slate-200 flex flex-col space-y-4 sm:space-y-6 animate-fade-in w-full max-w-7xl mx-auto px-1.5 sm:px-4">
      <style>{`
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 border-b border-slate-100 pb-4 sm:pb-5">
        <div className="flex items-center gap-2 sm:gap-3">
          <button onClick={onPrevMonth} className="p-2 sm:p-2.5 rounded-full hover:bg-slate-100 transition-colors text-slate-500 hover:text-indigo-600 active:scale-95 border border-slate-100 bg-white">
            <Icon name="chevronLeft" className="w-4 h-4 sm:w-5 sm:h-5" />
          </button>
          <div>
            <span className="text-[9px] sm:text-xs font-black tracking-widest text-slate-400 block uppercase">Calendar Explorer</span>
            <h2 className="text-xl sm:text-3xl font-black text-slate-800 tracking-tight">
              {year}年 {month + 1}月
            </h2>
          </div>
          <button onClick={onNextMonth} className="p-2 sm:p-2.5 rounded-full hover:bg-slate-100 transition-colors text-slate-500 hover:text-indigo-600 active:scale-95 border border-slate-100 bg-white">
        ...
        <div className="flex flex-wrap items-center gap-1.5 sm:gap-3">
          <div className="bg-slate-50 border border-slate-200/60 px-2.5 py-1 sm:px-3.5 sm:py-1.5 rounded-xl font-bold text-[10px] sm:text-xs text-slate-600 flex items-center gap-1.5 flex-grow sm:flex-grow-0 justify-center">
            <span className="w-1.5 h-1.5 rounded-full bg-indigo-500"></span>
            <span>今月予定: <strong className="text-indigo-600 font-extrabold">{monthlyTasks.length}</strong></span>
          </div>
          <div className="bg-green-50/50 border border-green-200/50 px-2.5 py-1 sm:px-3.5 sm:py-1.5 rounded-xl font-bold text-[10px] sm:text-xs text-green-700 flex items-center gap-1.5 flex-grow sm:flex-grow-0 justify-center">
            <span className="w-1.5 h-1.5 rounded-full bg-green-500"></span>
            <span>完了: <strong className="text-green-600 font-extrabold">{completedCount}</strong></span>
          </div>
          <div className="bg-rose-50/50 border border-rose-200/50 px-2.5 py-1 sm:px-3.5 sm:py-1.5 rounded-xl font-bold text-[10px] sm:text-xs text-rose-700 flex items-center gap-1.5 flex-grow sm:flex-grow-0 justify-center">
            <span className="w-1.5 h-1.5 rounded-full bg-rose-500 animate-pulse"></span>
            <span>提出: <strong className="text-rose-600 font-extrabold">{pendingDeadlinesCount}</strong></span>
          </div>
        </div>
      </div>

      {/* Advanced Quick Filters Control Board */}
      <div className="bg-slate-50/50 p-3 sm:p-4 rounded-xl sm:rounded-2xl border border-slate-100 flex flex-col gap-3 sm:gap-4">
        {/* Filtering by Task Type */}
        <div className="flex flex-col gap-2.5 sm:flex-row sm:items-center sm:justify-between">
          <div className="flex flex-wrap items-center gap-1.5">
            <button
              onClick={() => setTypeFilter('all')}
              className={`px-2.5 py-1 sm:px-3.5 sm:py-1.5 rounded-lg text-[10px] sm:text-xs font-black transition-all ${
                typeFilter === 'all' 
            ...
            <button
              ...
              className={`px-2.5 py-1 sm:px-3.5 sm:py-1.5 rounded-lg text-[10px] sm:text-xs font-black transition-all flex items-center gap-1 ${
                typeFilter === 'study' 
                  ? 'bg-indigo-600 text-white shadow-sm' 
          ...
          <button
            ...
            className={`px-2.5 py-1 sm:px-3.5 sm:py-1.5 rounded-lg text-[10px] sm:text-xs font-black border transition-all flex items-center justify-center gap-1.5 ${
              hideCompleted 
        ...
        <div className="flex flex-wrap items-center gap-1.5 border-t border-slate-100 pt-2.5">
          <span className="text-[9px] uppercase font-black text-slate-400 mr-1 sm:mr-2">教科別:</span>
          <button
            onClick={() => setSubjectFilter('all')}
            className={`px-2.5 py-0.5 sm:px-3 sm:py-1 rounded-full text-[10px] sm:text-xs font-black transition-all ${
              subjectFilter === 'all' 
          ...
          {subjects.map(sub => {
            ...
            return (
              <button
                ...
                className={`px-2.5 py-0.5 sm:px-3 sm:py-1 rounded-full text-[10px] sm:text-xs font-bold transition-all flex items-center gap-1 relative ${
      ...
      {/* Week Header */}
      <div className="grid grid-cols-7 gap-1 sm:gap-2 text-center font-black text-slate-400 text-[10px] sm:text-xs flex-shrink-0 mb-1 px-1">
      ...
      {/* Grid Canvas body */}
      <div className="grid grid-cols-7 gap-1 sm:gap-2.5 bg-slate-100/60 p-1 sm:p-3 rounded-2xl sm:rounded-[2.5rem] border border-slate-200 min-h-[350px] sm:min-h-[550px]">
        {calendarCells.map((cell) => {
          ...
          if (cell.type !== 'current') {
            ...
            containerClass = "bg-white ring-2 sm:ring-4 ring-indigo-500/80 ring-offset-1 sm:ring-offset-2 z-10 shadow-md shadow-indigo-100/30";
          }
          ...
          return (
            <div
              key={cell.key}
              className={`rounded-xl sm:rounded-2xl p-0.5 sm:p-2 sm:pb-3.5 flex flex-col relative group cursor-pointer transition-all duration-200 min-h-[50px] sm:min-h-[85px] border ${containerClass} ${hoverClass}`}
              onClick={() => {
            ...
            >
              ...
              <div className="flex justify-between items-start mb-0.5 sm:mb-1">
                <span className={`text-[10px] sm:text-xs font-black flex-shrink-0 w-4.5 h-4.5 sm:w-7 sm:h-7 flex items-center justify-center rounded-full transition-all ${isToday ? 'bg-indigo-600 text-white font-black scale-105' : dateTextColor}`}>
                  {day.getDate()}
                </span>
                
                {/* Mobile dots indicator - Colorful subject points */}
                {filteredTasksForDay.length > 0 && cell.type === 'current' && (
                  <div className="flex flex-wrap gap-0.5 justify-center sm:hidden mt-1.5 mr-0.5 max-w-[24px] overflow-hidden">
                    {filteredTasksForDay.slice(0, 3).map(task => {
                      ...
                      return (
                        <span 
                          ...
                          style={{ backgroundColor: sub?.color || '#ccd0d5' }}
                        />
                      );
                    })}
                  </div>
